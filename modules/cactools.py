...
def redirect(bot,trigger):
    sendto={bot.config.core.owner}
    if bot.memory.get('cactrack_name', False):
        sendto.add(bot.memory['cactrack_name'])
    for u in sendto:
        bot.notice("!hilight! %s in %s : %s" % (trigger.nick, trigger.sender, trigger.group(1)), u)#more magic
...
def redirpc(bot,trigger):
    if(trigger.nick == trigger.sender) or (trigger.group(0)[0]=='.' and trigger.group(0)[:2]!='..'):
        ...
        if bot.memory.get('cactrack_name', False):
            sendto.add(bot.memory['cactrack_name'])
        for u in sendto:
            bot.msg(u, "%s in %s : %s" % (trigger.nick, trigger.sender, trigger.group(1)))
